...
* <b>rawData</b> - Holds frame data on buffer format for convinience.
* <b>err</b> - Marks the frame as an error frame. canid will hold the error code in this case. Normally no data.

The timestamp (in microseconds) is generated by the node if not supplied by the interface.

Error codes are defined in the Linux kernel header file [can/error.h](https://github.com/torvalds/linux/blob/master/include/uapi/linux/can/error.h)
